...
import xmpp.util.sreenshot.CropImageView;
...
//		Log.i(TAG, "得到的图片的路径是 = " + mPath);
		...
		isFromChoose = getIntent().getBooleanExtra("isfrom_choose", false);
		try {
...
//			Tool.initToast(CropImageActivity.this, "图像处理中,请稍候...");
	...
	public void onClick(View v) {
		int i = v.getId();
		if (i == R.id.gl_modify_avatar_cancel) {// mCrop.cropCancel();
			finish();

		} else if (i == R.id.gl_modify_avatar_save) {
		...
		} else if (i == R.id.gl_modify_avatar_rotate_left) {
			mCrop.startRotate(270.f);

		} else if (i == R.id.gl_modify_avatar_rotate_right) {
	...
	public Bitmap createBitmap(String path, int w, int h) {
		try {
			...
			newOpts.inSampleSize = (int) ratio + (isFromChoose ? 1 : 2);
