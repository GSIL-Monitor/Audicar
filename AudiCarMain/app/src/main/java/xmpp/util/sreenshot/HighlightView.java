...
//                    mResizeDrawableHeight.draw(canvas);
    ...
    void moveBy(float dx, float dy) {
    	...
    	float width_pro = 1.0f ;   //�������               ,��ֹ��ͼ����
    	if(PicSrcPickerActivity.WIDTH_PROPOR!=1.0f)
    		width_pro = width_pro + (PicSrcPickerActivity.WIDTH_PROPOR-1.0f)*2;
        // Put the cropping rectangle inside image rectangle.
        mCropRect.offset(Math.max(0, mImageRect.left - mCropRect.left), Math.max(0, mImageRect.top - mCropRect.top));

        mCropRect.offset(Math.min(0, mImageRect.right - mCropRect.right*width_pro-10), Math.min(0, mImageRect.bottom - mCropRect.bottom));
...
//            r.inset(0F, -(heightCap - r.height()) / 2F);
    ...
    public Rect getCropRect() {
    	float width_pro = 1.0f ;   //�������
    	if(PicSrcPickerActivity.WIDTH_PROPOR!=1.0f)
    		width_pro = PicSrcPickerActivity.WIDTH_PROPOR;
        return new Rect((int) mCropRect.left, (int) mCropRect.top, (int)( mCropRect.right*width_pro), (int) mCropRect.bottom);
    }
    ...
    private Rect computeLayout() {
		...
		RectF r = new RectF(mCropRect.left, mCropRect.top, mCropRect.right*width_pro, mCropRect.bottom);
        mMatrix.mapRect(r);
    ...
    public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean circle, boolean maintainAspectRatio) {
    	...
    	if(PicSrcPickerActivity.WIDTH_PROPOR!=1.0f)
    		width_pro = PicSrcPickerActivity.WIDTH_PROPOR;
        mCropRect = cropRect;
        mImageRect =new RectF(imageRect.left,imageRect.top,imageRect.right*width_pro,imageRect.bottom);
